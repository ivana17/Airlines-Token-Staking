...
Your task is to create a minimal version of a staking platform. User should be able to stake whitelisted tokens (it’s up to you which one, but MVPW Airline Token must be among whitelisted ones). You should specify APY rewards percentage inside the smart contract. User should be able to claim its rewards at any particular moment. User should be able to withdraw its stake. Time vesting period is three months. That means that if User withrdaws its stake before that period, withdrawal slash is 50% (If user stake 100$ of token, if it try to withdraw before the three months vesting period, it will withdraw 50$ of token).

#### Task Goals:

Learning about Foundry
...
Your task is to write tests for Staking contract in Solidity. Your task is to optimize the gas usage of your contract. You should provide gas snapshots. There is a special reward for the one with the most optimized written contract.

#### Task Goals:

Learning about writing tests in Foundry
